...
def _fake_storage_provider(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    """Documents tests never talk to real S3/MinIO: STORAGE_PROVIDER is
    forced to "fake" and the process-wide provider singleton is reset
    before and after every test, so each test gets its own clean
    in-memory FakeStorageProvider instance rather than leaking uploaded
    objects between tests.
    """
...
def _fake_embedding_provider(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    """Documents tests never talk to the real Voyage API: EMBEDDING_PROVIDER
    is forced to "fake" (a deterministic, network-free provider -- see
    app.core.embeddings.fake_provider) and the process-wide provider
    singleton is reset before and after every test.
    """
