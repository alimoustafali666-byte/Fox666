"""Fixtures for the Step 14 Arabic/multilingual RAG pipeline-validation
suite. Identical isolation pattern to every other test package in this
repo (tests/conversations/conftest.py in particular, which this mirrors
line for line): a savepoint-rollback db_session so nothing this suite
does ever touches the shared database beyond its own test, and Fake
storage/embedding/LLM providers forced on so no real S3/Voyage/Anthropic
credentials or network access are ever required.

This suite validates PIPELINE correctness only (upload -> parse ->
normalize -> chunk -> embed -> retrieve -> ground -> respond, for
Arabic/bilingual content specifically) -- never real Arabic semantic
retrieval or generation quality, which remains an unverified,
pre-production, live-provider requirement (see each test module's
module docstring and the Step 14 report).
"""
