...
def fake_llm_provider(monkeypatch: pytest.MonkeyPatch) -> Iterator[FakeLLMProvider]:
    """Daily Brief tests never talk to the real Anthropic API: LLM_PROVIDER
    is forced to "fake" (app.core.llm.fake_provider) and the process-wide
    provider singleton is reset before and after every test. Yields the
    FakeLLMProvider instance itself so a test can `.enqueue_brief(...)` a
    scripted response/error, or inspect `.brief_calls` to prove (or
    disprove) that the provider was invoked at all.
    """
